...
#' (default 0); the rest will be doublets created across clusters. Ignored if `clusters`
#' is NULL.
...
#' @param meta.triplets Logical; whether to create triplets from meta cells. Ignored if 
#' `clusters` is missing.
#'
#' @return A count matrix for artificial doublets.
#' 
#' @export
getArtificialDoublets <- function(x, n=3000, prop.fullyRandom=0, clusters=NULL, n.meta.cells=1, meta.triplets=TRUE){
  library(Matrix)
  ...
  m2 <- x[,ca[,1]]+x[,ca[,2]]
  colnames(m2) <- paste0("arificialDoublet.",ifelse(is.null(ad.m),0,ncol(ad.m))+1:ncol(m2))
  if(is.null(ad.m)){
    ad.m <- m2
  }else{
    ad.m <- cbind(ad.m, m2)
  ...
  if(meta.triplets){
    ...
    if(length(unique(clusters))^3 > n){
      ...
      warning("Too many clusters - will create triplets only for the ", topc," largest clusters.")
      tt <- names(tt)[order(tt,decreasing=TRUE)[1:topc]]
