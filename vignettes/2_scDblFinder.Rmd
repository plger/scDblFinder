...
In this way, `r Githubpkg("satijalab.org/seurat")` clustering could for instance be used to create the artificial doublets (see `?Seurat::as.SingleCellExperiment.Seurat` for conversion to SCE).

### Can this be used with scATACseq data?

We have not yet thoroughly tested the parameters in the context of scATACseq data, however preliminary results on a couple of datasets (applied on peak-level counts) suggest that it works decently there. However, since scATACseq data normally have more peaks with fewer reads, we suggest increasing the `nfeatures` used.